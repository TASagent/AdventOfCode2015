...
Console.WriteLine("Day 21 - RPG Simulator 20XX");
...
int bossDamage = int.Parse(Regex.Match(lines[1], @"\d+").Value);
int bossArmor = int.Parse(Regex.Match(lines[2], @"\d+").Value);
...
int GetMinGoldCost(int atk, int def, Item[] weapons, Item[] armors, Item[] rings)
{
    ...
    foreach (Item weapon in weapons)
    {
        ...
        foreach (Item armor in armors)
        {
            ...
            int cost = weapon.cost + armor.cost + GetMinGoldCostRings(
...
int GetMinGoldCostRings(int atk, int def, int ringCount, int ringIndex, Item[] rings)
{
    if (atk == 0 && def == 0)
    {
        return 0;
    ...
    for (int i = ringIndex; i < rings.Length; i++)
    {
        ...
        {
            int cost = rings[i].cost + GetMinGoldCostRings(
...
int GetMaxGoldCost(int atk, int def, Item[] weapons, Item[] armors, Item[] rings)
{
    int maxGoldCost = 0;
    ...
    foreach (Item weapon in weapons)
    {
        ...
        foreach (Item armor in armors)
        {
            ...
            int cost = weapon.cost + armor.cost + GetMaxGoldCostRings(
                atk: atk - weapon.damage,
                def: def - armor.armor,
                ringCount: 0,
                ringIndex: 0,
...
int GetMaxGoldCostRings(int atk, int def, int ringCount, int ringIndex, Item[] rings)
{
    if (atk == 0 && def == 0)
    {
        return 0;
    ...
    for (int i = ringIndex; i < rings.Length; i++)
    {
        if (atk - rings[i].damage >= 0 && def - rings[i].armor >= 0)
        {
            int cost = rings[i].cost + GetMaxGoldCostRings(
                atk: atk - rings[i].damage,
                def: def - rings[i].armor,
                ringCount: ringCount + 1,
                ringIndex: i + 1,
